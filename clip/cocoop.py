...
class CoCoOpPromptLearner(nn.Module):
    # 初始化模型，接受 CLIP 模型、类别名称列表、上下文维度（n_ctx）、上下文初始化字符串（ctx_init）和类别标记位置（ctx_position）。
    def __init__(self, clip_model, classnames, n_ctx=4, ctx_init="a_photo_of_a", ctx_position='end'):
        ...
        # 上下文向量初始化: 如果提供了 ctx_init，则使用提供的字符串初始化上下文向量；否则，随机初始化。
        if ctx_init:
    ...
    # 根据图像特征和上下文向量生成提示，可以选择只生成上下文（ctx_only）。
    def forward(self, im_features, ctx_only=False):
        ...
        return prompts
# 初始化 (__init__): 加载 CLIP 模型，创建图像编码器、文本编码器和提示生成器。
# 推理 (inference): 对给定的图像进行推理，生成图像特征和文本特征，然后计算 logits。
# 生成上下文 (gen_ctx): 根据图像特征生成上下文向量。
# 前向传播（上下文） (forward_ctx): 给定图像特征和上下文向量，生成提示并计算 logits。
# 前向传播 (forward): 根据输入类型（元组或非元组）调用适当的方法。
